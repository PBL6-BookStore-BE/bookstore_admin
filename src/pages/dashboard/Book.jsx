...
import {
  ...
  toggleModalAdd,
  toggleModalDelBook,
} from "../../store/cases/book/slice";
import {
  listAuthors,
  listBooks,
  listCategories,
  listPublishers,
} from "../../store/cases/getAll/action";
...
const Book = () => {
  ...
  const { books, categories, authors, publishers } = useSelector(
    (state) => state.getAll
  );
      ...
      if (filter.search === "" && filter.category.value === "") {
        ...
        data = data.filter((element) =>
          element.categoryName
            .toLowerCase()
            .includes(filter.category.label.toLowerCase())
        );
        setBookData(data);
      } else if (filter.category.value !== "" && filter.search !== "") {
        data = data.filter(
                  ...
                  dispatch(
                    setEditBook({
                      ...
                      book: {
                        ...
                        idCategory: getId(
                          book.categoryName,
                          categories.dataInSelect
                        ),
                        idPublisher: getId(
                          book.publisherName,
                          publishers.dataInSelect
                        ),
                        idAuthors: getIdAuthors(
                          book.authors,
                          authors.dataInSelect
                        ),
                        list_img: book.urls,
            ...
            <Tooltip label="Delete" placement="top">
              <Button
                ...
                onClick={(e) => {
                  ...
                  dispatch(
                    setEditBook({ bookId: book.id, nameBook: book.name })
                  );
                }}
              >
                <DeleteIcon />
  ...
  return (
    <div>
      <VStack align="flex-start" spacing={8}>
        ...
        {bookData.length === 0 ? (
          <Heading size="md" fontWeight="600" color="#8D28AD">
            No book to display.....
          </Heading>
        ) : (
          <Table
      ...
      <FormAddBook
        categories={categories.dataInSelect}
        authors={authors.dataInSelect}
        publishers={publishers.dataInSelect}
      />
      <ModalDelete />
    </div>
  );
